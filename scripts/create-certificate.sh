...
# Path to the custom Lemp $(hostname) Root CA certificate.
PATH_ROOT_CNF="${PATH_SSL}/ca.lemp.$(hostname).cnf"
PATH_ROOT_CRT="${PATH_SSL}/ca.lemp.$(hostname).crt"
PATH_ROOT_KEY="${PATH_SSL}/ca.lemp.$(hostname).key"
...
then
    ...
    # Finally, generate the private key and certificate signed with the Lemp $(hostname) Root CA.
